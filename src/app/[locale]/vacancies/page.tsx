    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 px-8 md:px-14 py-16 items-start">
        <div className="flex flex-col gap-5">
          <div className="text-[11px] tracking-[0.18em] uppercase font-bold text-forest-light mb-1 flex items-center gap-3">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-text-dim border border-border rounded-lg">Нет активных вакансий</div>
          ) : (
            vacancies.map((v, i) => {
              ...
              return (
                <Link key={i} href={`/vacancies/${v.id}`} className="no-underline group">
                  <article className="border border-border bg-white rounded-xl p-6 md:p-7 hover:shadow-xl hover:-translate-y-0.5 transition-all">
                    <div className="flex gap-2 mb-3 flex-wrap">
                      {v.isHot && <span className="bg-red-500/10 text-red-500 text-[10px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-sm">{t('vacanciesPage.hotVacancy')}</span>}
                      <span className="bg-sky text-forest-light text-[10px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-sm">📍 {v.location || 'Республика Казахстан'}</span>
                    </div>
                    <h2 className="text-[20px] font-extrabold text-forest leading-snug mb-2 group-hover:text-amber-dark transition-colors">{title}</h2>
                    {desc && <p className="text-[13.5px] text-text-dim leading-relaxed line-clamp-2 mb-4">{desc}</p>}
                    <div className="flex items-end justify-between gap-4 pt-4 border-t border-border">
                      <div>
                        <div className="text-[10px] text-text-dim font-bold tracking-widest uppercase mb-0.5">{t('vacanciesPage.salary')}</div>
                        <div className="text-[15px] font-extrabold text-forest">{v.salary || t('vacanciesPage.byAgreement')}</div>
                      </div>
                      <span className="text-[12px] font-bold text-forest-light inline-flex items-center gap-1 group-hover:gap-2 transition-all whitespace-nowrap">{t('common.readMore')} →</span>
                    </div>
                  </article>
                </Link>
              );
        ...
        {/* Sidebar Info */}
        <aside className="bg-sky p-8 rounded-xl">
          <div className="w-12 h-12 bg-white flex items-center justify-center text-2xl mb-5 shadow-sm rounded-sm">🌲</div>
          ...
          <div className="pt-6 border-t border-border">
            ...
            <a href="tel:+77273461371" className="text-[15px] font-bold text-forest no-underline inline-block">+7 (727) 346-13-71</a>
          </div>
        </aside>
      </div>
    </>
